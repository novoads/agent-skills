...
[Novoads](https://novoads.ai/en?utm_source=claude-code&utm_medium=github&utm_campaign=skill-pack)
...
Everything below was made by this pack through the same skills a customer installs — the first
run on 2026-08-11/12, the image-to-motion row re-rendered on 2026-08-15. Each item shows the exact ask typed to the agent and the prompt the skill wrote
...
| [`skills/image-to-motion/`](skills/image-to-motion/) | Animate a finished still as-is. Motion vocabulary for UI, marketing hero, flat-lay, key art, collage, product and character images, plus the prompt clauses that pin camera, timing, frozen regions and text fidelity, so printed words stay legible instead of being redrawn. |
| [`skills/pixar-ad/`](skills/pixar-ad/) | A product URL or photo → a finished stylized 3D animated ad, built as a reviewed storyboard: one clip per beat, narrator voice-over layered into the gaps, then local assembly. Stills chain by `assetId`, so holding one character across separately-rendered beats needs no re-upload. **Every Pixar-style ask lands here, at any length.** [`references/formulas.md`](skills/pixar-ad/references/formulas.md) holds the four genre roles with a variable table and a worked still and clip prompt for each; [`evals.md`](skills/pixar-ad/evals.md) the scored cases; [`references/NOTICE.md`](skills/pixar-ad/references/NOTICE.md) the upstream MIT notice for the craft it adapts. |
| [`skills/claymation-ad/`](skills/claymation-ad/) | The clay sibling. Same pipeline as the Pixar skill; what it owns is the hand-sculpted clay look and a longer, quieter arc built on a named protagonist. [`references/formulas.md`](skills/claymation-ad/references/formulas.md) holds the eight beats, the subject-lock fragments and the clay QA checklists; [`evals.md`](skills/claymation-ad/evals.md) the clay-specific cases on top of the Pixar skill's; [`references/NOTICE.md`](skills/claymation-ad/references/NOTICE.md) the upstream MIT notice. |
