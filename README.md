...
<!-- The skills.sh count badge stays parked, and on 2026-08-14 the reason for
     ...
     current ones plus 6 names this repo renamed away (pixar-ad,
     claymation-ad, image-to-motion, image-ad-clone,
...
Thirteen skills, plus the shared steps they call, make the ad and the things around it. The generated
creative runs on [nine models](#supported-models), six for video and three for stills: UGC video,
static Meta creatives, Pixar and claymation storyboards, narrated 3D explainer ads, and YouTube
thumbnails. The narrator
...
| **Zack-D-style 3D explainer ad** | The science-lesson flow: a narrated 8-block script (hook → agitation → villain → mechanism → proof → product → dream outcome → CTA) split into one scene per spoken line, each scene a `nano-banana-pro` still animated as its own start-image clip, cut to a single `POST /v1/voiceovers` take whose Whisper timestamps are the edit map. Human-first hook, product invisible until its reveal, no text in any frame, two-token accent system. Gated twice: the script is approved before any charge, the stills before any clip | [zack-d-style-ad](skills/zack-d-style-ad/SKILL.md) · formulas in [references/formulas.md](skills/zack-d-style-ad/references/formulas.md) |
...
| [`skills/zack-d-style-ad/`](skills/zack-d-style-ad/) | A Zack-D-style animated science-explainer ad: script first (eight blocks, approved on its own), one still per spoken line with the character anchor and product pinned as references, each still animated as its own start-image clip, one voiceover take timed by Whisper into the cut map, music ducked under. [`references/formulas.md`](skills/zack-d-style-ad/references/formulas.md) holds the genre rules — the sandwich, accent discipline, scene-brief patterns, the assembly recipe; [`evals.md`](skills/zack-d-style-ad/evals.md) records five cases, each from a measured failure in the build that produced the skill. |
